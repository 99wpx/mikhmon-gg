...
if (!isset($_SESSION["mikhmon"])) {
    ...
    // Ambil log dari hotspot dan ppp
    $hotspotLog = $API->comm("/log/print", [
        "?topics" => "hotspot,info,debug"
    ]);
    $pppLog = $API->comm("/log/print", [
        "?topics" => "ppp,info,debug"
    ]);

    // Gabungkan dan urutkan berdasarkan waktu
    $getlog = array_merge($hotspotLog, $pppLog);
    usort($getlog, function ($a, $b) {
        return strcmp($b['time'], $a['time']); // descending
    });
  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        <h3><i class="fa fa-align-justify"></i> <?= $_hotspot_log ?> &nbsp; | &nbsp;&nbsp;
